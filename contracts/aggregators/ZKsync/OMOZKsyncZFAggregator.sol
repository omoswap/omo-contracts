...
import "./libraries/ZKsyncZFLibrary.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract OMOZKsyncZFAggregator is Ownable {
    ...
    address public factory = 0x3a76e377ED58c8731F9DF3A36155942438744Ce3;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address initialPair = ZKsyncZFLibrary.pairFor(factory, path[0], path[1]);
        IERC20(path[0]).safeTransferFrom(msg.sender, initialPair, amountIn);
    ...
    function _swapSupportingFeeOnTransferTokens(address initialPair, address[] calldata path, address _to) internal {
        for (uint i; i < path.length - 1; ) {
            (address input, address output) = (path[i], path[i + 1]);

            IZFPair pair = IZFPair(i == 0 ? initialPair : ZKsyncZFLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            { // scope to avoid stack too deep errors
                ...
                amountOutput = ZKsyncZFLibrary.getAmountOut(amountInput, reserveIn, reserveOut, swapFee);
            }

            address to = i < path.length - 2 ? ZKsyncZFLibrary.pairFor(factory, output, path[i + 2]) : _to;
