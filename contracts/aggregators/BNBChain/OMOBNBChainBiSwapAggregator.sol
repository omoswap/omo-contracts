...
import "./libraries/BNBChainBiswapLibrary.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract OMOBNBChainBiSwapAggregator is Ownable {
    ...
    address public factory = 0x858E3312ed3A876947EA49d572A7C42DE08af7EE;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        IERC20(path[0]).safeTransferFrom(
            msg.sender, BNBChainBiswapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(BNBChainBiswapLibrary.pairFor(factory, path[0], path[1]), amountIn));
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = BNBChainBiswapLibrary.sortTokens(input, output);
            IBiswapPair pair = IBiswapPair(BNBChainBiswapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = BNBChainBiswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, pair.swapFee());
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? BNBChainBiswapLibrary.pairFor(factory, output, path[i + 2]) : _to;
