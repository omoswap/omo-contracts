...
import "./libraries/BlasterswapV2Library.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract OMOBlastBlasterswapV2Aggregator is Ownable {
    ...
    address public factory = 0x9CC1599D4378Ea41d444642D18AA9Be44f709ffD;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        IERC20(path[0]).safeTransferFrom(
            msg.sender, BlasterswapV2Library.pairFor(factory, path[0], path[1]), amountIn
        );
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(BlasterswapV2Library.pairFor(factory, path[0], path[1]), amountIn));
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (address token0,) = BlasterswapV2Library.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(BlasterswapV2Library.pairFor(factory, input, output));
            uint256 amountInput;
            ...
            { // scope to avoid stack too deep errors
                ...
                amountOutput = BlasterswapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) = input == token0 ? (uint256(0), amountOutput) : (amountOutput, uint256(0));
            address to = i < path.length - 2 ? BlasterswapV2Library.pairFor(factory, output, path[i + 2]) : _to;
