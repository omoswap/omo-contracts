...
import "./libraries/PolygonQuickSwapV2Library.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract OMOPolygonQuickswapV2Aggregator is Ownable {
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        IERC20(path[0]).safeTransferFrom(
            msg.sender, PolygonQuickSwapV2Library.pairFor(factory, path[0], path[1]), amountIn
        );
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(PolygonQuickSwapV2Library.pairFor(factory, path[0], path[1]), amountIn));
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = PolygonQuickSwapV2Library.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(PolygonQuickSwapV2Library.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = PolygonQuickSwapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? PolygonQuickSwapV2Library.pairFor(factory, output, path[i + 2]) : _to;
