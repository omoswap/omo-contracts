...
import "./libraries/EthereumShibaLibrary.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract OMOEthereumShibaAggregator is Ownable {
    ...
    address public factory = 0x115934131916C8b277DD010Ee02de363c09d037c;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        IERC20(path[0]).safeTransferFrom(
            msg.sender, EthereumShibaLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(EthereumShibaLibrary.pairFor(factory, path[0], path[1]), amountIn));
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = EthereumShibaLibrary.sortTokens(input, output);
            IShibaPair pair = IShibaPair(EthereumShibaLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = EthereumShibaLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, pair.totalFee());
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? EthereumShibaLibrary.pairFor(factory, output, path[i + 2]) : _to;
