...
import "./libraries/MetisNetswapLibrary.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

interface INetswapFactory is IUniswapV2Factory {
    function feeRate() external view returns (uint);
}

contract OMOMetisNetswapAggregator is Ownable {
    ...
    address public factory = 0x70f51d68D16e8f9e418441280342BD43AC9Dff9f;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        IERC20(path[0]).safeTransferFrom(
            msg.sender, MetisNetswapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(MetisNetswapLibrary.pairFor(factory, path[0], path[1]), amountIn));
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = MetisNetswapLibrary.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(MetisNetswapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = MetisNetswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, INetswapFactory(factory).feeRate());
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? MetisNetswapLibrary.pairFor(factory, output, path[i + 2]) : _to;
